...
def compute_total_fund_performance(start_date: str, end_date: str) -> pl.DataFrame:
    ...
    fund_df = (
        ...
        .with_columns(pl.col("xs_div_return", "xs_return_bmk", "div_return").fill_null(0))
    )
    ...
    performance = (
        ...
        # )
        .with_columns(
            pl.col('total_return').sub(pl.col('total_return_bmk').mul(pl.col('beta'))).alias('alphas')
        )
        # Ratios
        .with_columns(
...
def compute_fund_performance(fund: str, start_date: str, end_date: str) -> pl.DataFrame:
    ...
    performance = (
        ...
        # )
        .with_columns(
            pl.col('total_return').sub(pl.col('beta').mul(pl.col('total_return_bmk'))).alias('alpha')
        )
        # Ratios
        .with_columns(
            pl.col("expected_return")
...
def create_overall_fund_performance(start_date: str, end_date: str) -> None:
    ...
    (
        ...
        .save(f"results/overall_fund_performance_{end_date}.png")
    )
...
def compute_fund_holdings_performance(
    ...
    performance = (
        holdings_df
        .filter(pl.col('ticker').is_in(current_tickers))
        .sort(["ticker", "caldt"])
        # Cummulative features
        .with_columns(
            pl.col("div_return")
...
def create_holdings_performance(start_date: str, end_date: str) -> None:
    ...
    for fund in funds:
        ...
        (
            ...
            .fmt_number(["Beta"])
            .opt_stylize(style=3, color="gray")
            .save(f"results/holdings_performance_{fund}_{end_date}.png")
        )
